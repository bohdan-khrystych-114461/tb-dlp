...
async def _download_and_send(update: Update, url: str, force: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if not (INSTAGRAM_RE.search(url) or TIKTOK_RE.search(url)):
                return
        ...
        audio = sorted(f for f in all_files if f.suffix.lower() in AUDIO_EXTS)
        videos = sorted(
            f for f in all_files
            if f.suffix.lower() not in IMAGE_EXTS and f.suffix.lower() not in AUDIO_EXTS
        )
